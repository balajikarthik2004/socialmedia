...
const RightBar = () => {
  return (
    <div className='hidden lg:block col-span-4 overflow-y-scroll scroll-smooth no-scrollbar p-5 pl-4'>
        {/* friendRequests */}
        {friendRequests.length > 0 && <div className="bg-white rounded-lg shadow px-4 py-3 pb-1">
            ...
            {friendRequests.map((request) => {
                return <FriendRequest name={request.name} url={request.url} />
            })}
        </div>}
        {/* suggestions */}
        <div className={`${friendRequests.length && "mt-5"} bg-white rounded-lg shadow px-4 py-3 pb-1`}>
            ...
            {suggestions.map((item) => {
                return <Suggestion name={item.name} url={item.url} />
            })}
        </div>
    </div>
  )
