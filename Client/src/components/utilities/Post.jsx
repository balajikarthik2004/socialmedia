...
const Post = ({post}) => {
  ...
  return (
    <div className='bg-white mt-5 p-3 sm:p-4 rounded-lg shadow'>
        <div className='flex justify-between'>
            <div className='flex items-center gap-2'>
                <img src={post.profilePic} alt="" className='block h-9 w-9 sm:h-10 sm:w-10 rounded-full object-cover' />
                <div>
                    <p>{post.name}</p>
                    <p className='text-[0.7rem] text-[#00000081]'>{post.creationDate}</p>
                </div>
            </div>
            <MoreHorizIcon />
        </div>
        ...
        {openComments && <div>
            <div className='mt-3 flex gap-3 justify-between items-center'>
                ...
                <input type="text" placeholder='Write a comment' className='block w-full border border-gray-300 bg-transparent outline-none rounded p-2 text-sm' />
                <div className='pb-2 pt-1 pr-1.5 pl-2 rounded bg-blue-500 hover:bg-blue-600 text-white'><SendIcon /></div>
            </div>
